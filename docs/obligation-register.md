...
register/ includes one illustrative entry, pqc-transition-hndl, covering migration away from quantum-vulnerable public-key cryptography. It is an anticipatory case: for most organisations, the obligation is presently expressed through standards guidance and procurement expectations rather than statute. The entry therefore demonstrates how the register can represent an emerging obligation before it fully crystallises, while making its present status explicit.

Its scoring assumptions are illustrative and deliberately labelled as such. Each organisation substitutes its own retention period and a defensible capability assumption, recording the reasoning so that a reviewer can test the assumption rather than merely the conclusion.
